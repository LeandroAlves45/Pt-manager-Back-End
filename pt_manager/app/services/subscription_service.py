...
class SubscriptionService:
    ...
    def can_add_client(subscription: TrainerSubscription) -> tuple[bool, str]:
        ...
        if subscription.active_clients_count >= max_clients:
            ...
            return False, f"Limite de clientes atingido para o tier {subscription.tier.upper()}. {next_tier_msg}"
        return True, ""
...
def _get_upgrade_message(current_tier: str) -> str:
    ...
    if current_tier == SubscriptionTier.FREE:
        return "Para adicionar mais clientes, faça upgrade para o tier STARTER (20€/mês)."
    elif current_tier == SubscriptionTier.STARTER:
        return "Para adicionar mais clientes, faça upgrade para o tier PRO (40€/mês)."
    else:
        return ""